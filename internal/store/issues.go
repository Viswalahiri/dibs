...
const issueColumns = `id, repo_id, number, title, body, html_url, author,
	labels, assignees, comment_count, created_at, first_seen_at, state,
...
func scanIssue(sc interface{ Scan(...any) error }) (Issue, error) {
	...
	err := sc.Scan(&iss.ID, &iss.RepoID, &iss.Number, &iss.Title, &iss.Body,
		&iss.HTMLURL, &iss.Author, &labelsJSON, &assigneesJSON,
...
func (s *Store) Insert(ctx context.Context, iss Issue) (id int64, inserted bool, err error) {
	...
	row := s.db.QueryRowContext(ctx, `
		INSERT INTO issues (repo_id, number, title, body, html_url, author,
		                    labels, assignees, comment_count, created_at,
		                    first_seen_at, state, reject_reason)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(repo_id, number) DO NOTHING
		RETURNING id`,
		iss.RepoID, iss.Number, iss.Title, iss.Body, iss.HTMLURL, iss.Author,
		string(labels), string(assignees), iss.CommentCount,
...
func (s *Store) SaveVerdict(ctx context.Context, id int64, to State, reason RejectReason) error {
	return s.move(ctx, id, StateNew, to, reason, "", time.Time{})
}
...
// programming error caught at the call, not a silently wrong row. The update is
// conditional on `from`, so a worker whose lease expired mid-flight can finish
// and write its result harmlessly: it gets ErrNotClaimable instead of
// double-processing the row.
func (s *Store) move(ctx context.Context, id int64, from, to State, reason RejectReason,
	...
	if to == StateRejected && reason == "" {
		return fmt.Errorf("issue %d: a rejection needs a reason", id)
	}
